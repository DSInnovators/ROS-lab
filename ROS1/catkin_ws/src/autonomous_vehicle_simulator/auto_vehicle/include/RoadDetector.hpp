...
/**
 *@brief Definition of the RoadDetector class. It contains all the functions and
 *variables depicted in the
...
private:
    ...
    bool right_flag =
        false;         // Tells us if there's right boundary of lane detected
...
public:
    cv::Mat deNoise(const cv::Mat &inputImage)
        const; // Apply Gaussian blurring to the input Image
    cv::Mat edgeDetector(
        const cv::Mat &img_noise) const; // Filter the image to obtain only edges
    cv::Mat mask(const cv::Mat &img_edges)
        const; // Mask the edges image to only care about ROI
    const std::vector<cv::Vec4i> houghLines(
        const cv::Mat &img_mask); // Detect Hough lines in masked edges image
    const std::vector<std::vector<cv::Vec4i>>
    lineSeparation(const std::vector<cv::Vec4i> &lines,
                   const cv::Mat &img_edges); // Sprt detected lines by their
                                              // slope into right and left lines
    const std::string &getDirectionFromLines(
